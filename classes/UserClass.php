...
class UserClass{
    ...
    */
    ...
    require ("config/dbconnect.php");
        ...
        if(isset($_POST['login'])){
            ...
            if(!empty($user) && !empty($userpsw)){
                ...
                $stmt = $conn->prepare("SELECT name, password FROM Lecturer WHERE name = ?");
    ...
    private function forgotPassword(){
        ...
        $stmt = $conn->prepare("SELECT email FROM Lecturer WHERE email = ?");
        ...
        if ($result->num_rows != 0) {
            ...
            $stmt = $conn->prepare("UPDATE Lecturer SET fpassword_key = ? WHERE email = ?");
    ...
    public function newPassword(){
        ...
        $stmt = $conn->prepare("SELECT email,fpassword_key  FROM Lecturer WHERE email = ? AND fpassword_key = ?");
    ...
    private function updatePassword(){
        ...
        if($password3===$password2){
            ...
            if(!empty($password3) && !empty($email)){
                ...
                $stmt = $conn->prepare("UPDATE Lecturer SET password = ?, fpassword_key = ?  WHERE email = ? AND fpassword_key = ?");
                $clean_password_key = "";
        ...
        require ('config/dbconnect.php');
            ...
            if($password===$password2){
                ...
                if(!empty($username) && !empty($password) && !empty($email)){
                    ...
                    // Check if username or email is already taken.
                    $stmt = $conn->prepare("SELECT Name, email FROM Lecturer WHERE Name = ? OR email = ?");
                    ...
                    if ($result->num_rows != 0) {
                        ...
                        $stmt = $conn->prepare("INSERT INTO Lecturer (Name, email, designation, courseID, password) VALUES (?, ?, ?, ?, ?)");
                        $stmt->bind_param("sssss", $username, $email, $securing, $code, $password);
                        $stmt->execute();
                        $stmt->close();
    ...
    public function Verify(){
        ...
        if(isset($_GET['id']) && isset($_GET['code'])){
            ...
            $stmt = $conn->prepare("SELECT email FROM Lecturer WHERE email = ? and activation_code = ?");
            ...
            if($result->num_rows == 1){
                $stmt = $conn->prepare("UPDATE Lecturer SET is_activated = ? WHERE email = ? and activation_code = ?");
                $verified = 1;
